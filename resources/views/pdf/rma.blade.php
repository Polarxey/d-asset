...
<html>
<head>
    <title>RMA - {{ $asset->serial_number }}</title>
    <style>
        @page { margin: 0.8cm; }
        body {
...
<body>
    ...
    <table class="info-table">
        <tr>
            ...
            <td style="width: 2%;">:</td>
            <td class="fw-bold">{{ $asset->id_pa }}</td>
        </tr>
        <tr>
            <td>2.</td>
            ...
            <td>
                <div class="checkbox-container">
                    <span class="checkbox">{!! $asset->valuation_type == 'Ex-Project' ? '&#10003;' : '' !!}</span> Ex-Project
                </div>
                <div class="checkbox-container">
                    <span class="checkbox">{!! $asset->valuation_type == 'Dismantle' ? '&#10003;' : '' !!}</span> Dismantle
                </div>
                <div class="checkbox-container">
                    <span class="checkbox">{!! $asset->valuation_type == 'Rusak-L' ? '&#10003;' : '' !!}</span> Rusak-L
                </div>
                <div class="checkbox-container">
                    <span class="checkbox">{!! $asset->valuation_type == 'Rusak-TL' ? '&#10003;' : '' !!}</span> Rusak-TL
                </div>
            </td>
        </tr>
        <tr>
            ...
            <td>Tanggal</td>
            <td>:</td>
            <td>{{ request('tanggal_rma') ? \Carbon\Carbon::parse(request('tanggal_rma'))->format('d/m/Y') : date('d/m/Y') }}</td>
        </tr>
        <tr>
            <td>4.</td>
            <td>Lokasi asal</td>
            <td>:</td>
            <td>{{ $asset->lokasi_asal }}</td>
        </tr>
        <tr>
            <td>5.</td>
        ...
        <tr>
            ...
            <td>:</td>
            <td class="fw-bold">{{ $asset->material_number ?? '-' }}</td>
        </tr>
    </table>
