    ...
    <div class="col-md-7">
        ...
        <div class="card h-100">
            <div class="card-body text-center py-5 d-flex flex-column justify-content-center align-items-center" style="background:#0e1117;">
                ...
                <a href="{{ route('bundle.create') }}" class="btn btn-sm mt-4 px-4" style="background:#1f6feb; color:#fff; border:none; border-radius:6px; font-size:.82rem;">
                ...
                <div class="mb-4">
                    ...
                    <select name="bundle_id" id="bundleSelect" class="form-select form-select-sm"
                        ...
                        <option value="{{ $bundle->id }}"
                            data-items="{{ $bundle->items->map(fn($i) => '[S/N: ' . ($i->asset->serial_number ?? '-') . '] - ' . ($i->asset->nama_perangkat ?? '?'))->join('||') }}"
                            data-count="{{ $bundle->items->count() }}"
                            {{ request('bundle_id') == $bundle->id ? 'selected' : '' }}>
                            {{ $bundle->nama_paket }} — {{ $bundle->items->count() }} unit
                        </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="card-body p-0">
                ...
                <table class="table table-dark table-hover mb-0" style="font-size:.78rem;">
                    <tbody>
                        ...
                        <tr>
                            <td class="ps-4">
                                <div style="color:#58a6ff; font-weight:600; font-family:monospace;">{{ $rt->no_bstp }}</div>
                                <div style="color:#8b949e; font-size:.72rem;">{{ $rt->penerima }}</div>
                                <div style="color:#484f58; font-size:.72rem;">{{ \Carbon\Carbon::parse($rt->tanggal_serah)->format('d/m/Y') }}</div>
                            </td>
                            <td class="text-end pe-4 align-middle">
                                <a href="{{ route('transactions.pdf', $rt->id) }}"
